...
interface IDCAFactoryPairsHandler {
  /// @notice Emitted when both tokens are equal
  error IdenticalTokens();
  /// @notice Emitted when trying to create a pair that already exists
  ...
  /** @dev If the pair already exists, it raises the PairAlreadyExists error.
    ...
    If the oracle does not suppor the pair, raises the PairNotSupported error.
  */
  /// @param _tokenA first token of the pair
  /// @param _tokenB second token of the pair
