...
describe('DCAZRXSwapper', () => {
  ...
  before('Setup accounts and contracts', async () => {
    ...
    DCASwapperContract = await ethers.getContractFactory('contracts/mocks/DCASwapper/DCAZRXSwapper.sol:DCAZRXSwapperMock');
    ...
    function shouldBeSwappedTest({
      ...
      when(title, () => {
        ...
        then('findBestSwap returns as expected', async () => {
          if (shouldBeSwapped) {
            expect(decodeFeeTier(returnedResult)).to.be.equal(FEE_TIER);
          } else {
            expect(returnedResult).to.equal('0x');
          }
        });
      });
    }
