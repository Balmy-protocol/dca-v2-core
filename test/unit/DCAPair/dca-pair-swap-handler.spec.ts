  ...
  describe('_swap', () => {
    ...
    context('when last swap was > than swap interval ago', () => {
      ...
      it('performs swap', async () => {
        ...
        const initialToTokenBalance = await tokenB.balanceOf(
          DCAPairSwapHandler.address
        );
        await DCAPairSwapHandler.swap();
        ...
        expect(await tokenB.balanceOf(DCAPairSwapHandler.address)).to.be.equal(
          initialToTokenBalance.add(boughtToToken)
        );
        expect(await tokenB.balanceOf(DCAPairSwapHandler.address)).to.be.gt(
