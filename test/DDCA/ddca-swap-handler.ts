...
describe.only('DDCASwapHandler', function() {
  ...
  describe('_swap', () => {
    context('when last swap was < than swap interval ago', () => {
      ...
      it('reverts with message', async () => {
        await expect(
          DDCASwapHandler.swap()
        ).to.be.revertedWith('DDCASH: within swap interval');
      });
    });
    context('when some fonky stuff happend and wants to buy negative', () => {
      ...
      it('reverts with message', async () => {
        await expect(
          DDCASwapHandler.swap()
        ).to.be.revertedWith('DDCASH: amount should be > 0');
      });
    });
    context('when the addition does not overflow averages rates per unit', () => {
      // amount to swap
      ...
      beforeEach(async () => {
        ...
        await fromToken.transfer(DDCASwapHandler.address, uniswapRatePerUnit.mul(amountOfSwaps));
      });
    ...
    context('when the addition overflows averages rates per unit', () => {
      ...
      beforeEach(async () => {
        ...
        await DDCASwapHandler.setAverageRatesPerUnit(
          performedSwaps,
          [
